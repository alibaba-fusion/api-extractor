...
function generatePropMD(name, prop) {
  ...
  const verison = prop.version ? prop.version.value : '';
  const device = prop.device ? prop.device.value : '';

  return `${name} | ${description}${extra} | ${displayType} | ${defaultValue} | ${verison} | ${device}\n`;
}
